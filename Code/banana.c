...
Copyright (c) 2008 by Jeffrey S. Rosenthal
...
Compile with "cc banana.c -lm", then run with "a.out".

Upon completion, can run e.g. 'source("adaptx1")' in R to see a trace plot
...
#define X1FILE "adaptx1"
#define X2FILE "adaptx2"
#define RFILE "adaptR"
#define EFILE "adapte"
#define BFILE "adaptb"
#define AFILE "adapta"
...

{
    ...
    double Vmatrix[DIM][DIM];
    double ell, A, tmpsum, normal(), targlogdens(), sq();
    FILE *fpmath, *fpx1, *fpx2, *fpr, *fpe, *fpb, *fpa;
    ...
    /* COMPUTE INITIAL V MATRIX. */
    for (i=0; i<DIM; i++) {
      for (j=0; j<DIM; j++) {
	if ( (i==0) && (j==0) ) {
          Vmatrix[i][j] = 0.01 + 2*bananicity*(X[1]+bananicity*sq(X[0])
	  	- 100*bananicity) + 4*sq(bananicity*X[0]);
	} else if ( ((i==0) && (j==1)) || ((i==1) && (j==0)) ) {
          Vmatrix[i][j] = 2*bananicity*X[0];
	} else if (i==j) {
          Vmatrix[i][j] = 1.0;
	} else {
          Vmatrix[i][j] = 0.0;
    ...
    fprintf(fpb, "\nRrat<-function(x)\n");
    fprintf(fpb, "{\n");
    fprintf(fpb, "eigs<-eigen(x, only.values=TRUE)$values\n");
	...
	if (t > aspacing) /* Not first one printed, so need comma. */
      ...
      if (t == bspacing * (t/bspacing)) {
        ...
        /* UPDATE V MATRIX. */
        Vmatrix[0][0] = 0.01 + 2*bananicity*(X[1]+bananicity*sq(X[0])
	  	- 100*bananicity) + 4*sq(bananicity*X[0]);
        Vmatrix[0][1] = Vmatrix[1][0] = 2*bananicity*X[0];
    ...
    for (i=0; i<DIM; i++) {
      for (j=0; j<DIM; j++) {
        compmat[i][j] = 0.0;
        for (k=0; k<DIM; k++) {
          compmat[i][j] = compmat[i][j] + Vmatrix[i][k] * empcov[k][j];
        }
      }
    }
    ...
    printf("\nDone!  Details in files %s, %s, %s, %s, %s, and %s.\n\n",
			EFILE, X1FILE, X2FILE, RFILE, BFILE, MATHFILE);
    ...
    fprintf(fpr, "{\n");
    fprintf(fpr, "eigs<-eigen(x, only.values=TRUE)$values\n");
    ...
    fprintf(fpx1, " )\n");
    fprintf(fpx1, "\n# plot(x1vector, type='l')\n\n");
    fprintf(fpx2, " )\n");
    fprintf(fpx2, "\n# plot(x2vector, type='l')\n\n");
    fprintf(fpa, " )\n");
    fprintf(fpa, "\n# plot(avector, type='l')\n\n");
    fclose(fpmath);
    fclose(fpx1);
...
double targlogdens( double w[DIM] )
{
    ...
    tmpval = - sq(w[0])/200 - sq(w[1]+bananicity*sq(w[0])-100*bananicity)/2;
    for (ii=2; ii<DIM; ii++) {
        tmpval = tmpval - sq(w[ii]) / 2;
    }
